...
def process_pdfs(pdf_folder=f"materials/{CURRENT_CLASS}", vector_db_path="vector_store"):
    ...
    for file in os.listdir(pdf_folder):
        if file.endswith(".pdf"):
            ...
            for chunk in page_chunks:
                doc_text = chunk["text"]
                documents.append(doc_text)  # UNTOUCHED, to preserve structure for dense embeddings                
                metadata.append({"file": file, "page": chunk["page"]})
                #print(f"\n\n    📄 Document: {file} | Page: {chunk['page']}")
                
                # CLEANED for token extraction
                cleaned_text = clean_doc_text(doc_text)
                cleaned_text = tokenize_and_clean_text(cleaned_text)  # Tokenize and clean text
                #print(f"    🧹 Cleaned text: {cleaned_text}")
                              
                simple_tokens.append(cleaned_text) # For regular BM25 index with lemmatized tokens
                # Generate n-grams
    ...
    print(f"\n\n📚 Processed {len(documents)} documents from {pdf_folder}.\n")
    # Build BM25 Index
    ...
    #print(f"✅ {CURRENT_CLASS} Collection saved to {vector_db_path}/chroma.sqlite3")
    
    print(f"\n✅ Hybrid Search: Knowledge base created with vector & keyword search for {CURRENT_CLASS}.")
    print("   - Vector search: ChromaDB")
    print("   - Keyword search: BM25 index")
    print("   - Embedding model: all-MiniLM-L6-v2")
    print("   - BM25 model: rank_bm25\n")
